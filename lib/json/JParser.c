...
int JErr_setTooFewParams(JErr *o)
{
   return JErr_setError(o, JErrT_InvalidMethodParams, "Too few parameters");
}

int JErr_setTypeErr(JErr *o, JVType expT, JVType recT)
{
   if (o && JErr_noError(o))
   {
      o->expType = expT;
...
int JErr_setError(JErr *o, JErrT err, const char *msg)
{
   if (JErr_noError(o))
   {
      o->err = err;
...
#define JLexer_setBuf(o, buf, size)        \
   do                                      \
   {                                       \
   ...
   for (;;)
   {
      ...
      case JLexerSt_StringEscape:
         ...
         case 'v':
            ...
            case '\\':
               ...
               break;
            case 'b':
               asmB->buf[asmB->index] = '\b';
               break;
            case 'f':
               asmB->buf[asmB->index] = '\f';
               break;
            case 'n':
               asmB->buf[asmB->index] = '\n';
               break;
            case 'r':
               asmB->buf[asmB->index] = '\r';
               break;
            case 't':
               asmB->buf[asmB->index] = '\t';
               break;
            case 'v':
               asmB->buf[asmB->index] = '\v';
               break;
            }
            asmB->index++;
            o->tokenPtr++;
      ...
      case JLexerSt_StringUnicode:
      {
         ...
         { /* Convert unicode to UTF8. */
            if (JDBuf_expandIfNeeded(o->asmB, 4))
               return JLexerT_MemErr;
            if (o->unicode < 0x80)
            {
...
void JParser_constructor(JParser *o, JParserIntf *intf, char *nameBuf,
...
void JParser_destructor(JParser *o)
{
   JDBuf_destructor(&o->asmB);
}

int JParser_parse(JParser *o, const U8 *buf, U32 size)
{
   JLexerT lexerT;
            ...
            L_endObj:
               ...
               L_endParse:
                  return JParser_setStatus(
                      o, JLexer_hasMoreData(&o->lexer) ? JParsStat_Done : JParsStat_DoneEOS, 1);
               }
               o->state = JParserSt_Comma;
            }
            else
