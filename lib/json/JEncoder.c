...
int JEncoder_setInt(JEncoder *o, S32 val)
{
...
int JEncoder_setLong(JEncoder *o, S64 val)
{
   if (JEncoder_beginValue(o, FALSE))
   {
...
#ifndef NO_DOUBLE
int JEncoder_setDouble(JEncoder *o, double val)
{
   if (JEncoder_beginValue(o, FALSE))
   {
...
int JEncoder_setString(JEncoder *o, const char *val)
{
   if (JEncoder_beginValue(o, FALSE))
   {
      if (val)
...
int JEncoder_b64enc(JEncoder *o, const void *source, S32 slen)
{
   if (JEncoder_beginValue(o, FALSE))
   {
...
int JEncoder_vFmtString(JEncoder *o, const char *fmt, va_list argList)
{
   if (JEncoder_beginValue(o, FALSE))
   {
      if (fmt)
...
int JEncoder_setBoolean(JEncoder *o, BaBool val)
{
   if (JEncoder_beginValue(o, FALSE))
   {
...
int JEncoder_setNull(JEncoder *o)
{
   if (JEncoder_beginValue(o, FALSE))
   {
      if (BufPrint_write(o->out, "null", -1) < 0)
...
#define JEncoder_setJV(o, val, x)                                  \
   JErr_setError(o->err, JErrT_FmtValErr, "Feature 'J' Disabled"); \
   return -1
#else
int JEncoder_setJV(JEncoder *o, JVal *val, BaBool iterateNext)
{
...
int JEncoder_vSetJV(JEncoder *o, const char **fmt, va_list *argList)
{
   union
   {
      const void *p;
...
int JEncoder_set(JEncoder *o, const char *fmt, ...)
{
   int retVal;
   va_list varg;
   va_start(varg, fmt);
